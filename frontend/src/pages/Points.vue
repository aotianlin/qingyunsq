...
const rewards: RewardItem[] = [
  ...
  { title: 'CampusForum 定制马克杯', points: 1000, icon: GiftOutline, tone: 'cream', visual: 'mug' },
];
...
async function inviteFriend() {
  const inviteLink = typeof window === 'undefined' ? 'CampusForum' : `${window.location.origin}/register?invite=me`;
  try {
    ...
    <main class="points-main">
      <section class="hero-card glass-card" :class="{ boosted: boostActive }">
        ...
        <div class="level-strip">
          ...
          <div class="progress-track"><i :style="{ width: `${levelProgress}%` }"></i></div>
